...
const ChatList = () => {
  ...
  return (
    <div className="max-w-[390px] mx-auto bg-gray-50 p-4 min-h-screen">
      {/* 유저 타입이 "user"인 경우 "AI 트레이너" 채팅방 표시 */}
      {userType === 'user' && (
        <div
          onClick={() => handleClick('ai')}
          className="bg-gray-100 hover:bg-gray-200 transition rounded-lg p-4 mb-4 flex justify-between items-center cursor-pointer shadow-sm"
        >
          <span className="font-medium text-gray-700">AI 트레이너</span>
        ...
        [...chatRooms]
          ...
          .map((room) => (
            <div
              ...
              className="bg-white hover:bg-gray-100 transition rounded-lg p-4 mb-4 flex justify-between items-center cursor-pointer shadow-md"
            >
              <span className="font-medium text-gray-800">
                {room.other_party_name}{' '}
                {userType === 'user' ? '트레이너' : '회원'}
              </span>
              <span className="bg-blue-500 text-white text-xs py-1 px-3 rounded-full">
                new
              </span>
