...
function UserMypage() {
  ...
  const userId = useSelector((state) => state.auth); // auth 슬라이스에서 사용자 ID 가져오기

  console.log("현재 사용자 상태:", profile);
  console.log("현재 사용자 상태:", userId);

  useEffect(() => {
    dispatch(fetchUserProfile()); // userId가 올바르게 설정되었는지 확인
  }, [dispatch]);
